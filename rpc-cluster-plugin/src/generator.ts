...
export async function generate(ctl: GeneratorController, history: Chat): Promise<void> {
  ...
  let workers: Worker[];
  try {
    workers = await discoverWorkers(config.discoveryTimeoutMs);
  } catch (err) {
  ...
  const workersDisplay = formatDiscoveredWorkers(workers);
 ...
 */
export async function testCluster(ctl: GeneratorController): Promise<{
  success: boolean;
  response?: string;
  timeMs?: number;
  error?: string;
...
export default { generate, testCluster };
